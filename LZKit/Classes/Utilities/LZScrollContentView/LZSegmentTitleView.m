//
//  LZSegmentTitleView.m
//  LZKit
//
//  Created by 寕小陌 on 2017/4/28.
//  Copyright © 2017年 寕小陌. All rights reserved.
//

#import "LZSegmentTitleView.h"

@interface LZSegmentTitleView ()<UIScrollViewDelegate>
...
@property (nonatomic, assign) LZIndicatorType indicatorType;

@property (nonatomic, strong) NSArray *titlesArr;

@end

@implementation LZSegmentTitleView
...
- (void)initWithProperty
{
    self.itemMargin = 20;
    self.selectIndex = 0;
    self.titleNormalColor = textColorNormal;
    self.titleSelectColor = textColorSelected;
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [LZSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [LZSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [LZSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case LZIndicatorTypeDefault:
                ...
                break;
            case LZIndicatorTypeEqualTitle:
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(LZSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate LZSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
...
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(LZSegmentTitleViewWillBeginDragging:)]) {
        [self.delegate LZSegmentTitleViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(LZSegmentTitleViewWillEndDragging:)]) {
        [self.delegate LZSegmentTitleViewWillEndDragging:self];
    }
}
