...
@interface LZPageContentView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(LZContentViewWillBeginDragging:)]) {
        [self.delegate LZContentViewWillBeginDragging:self];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(LZContentViewDidScroll:startIndex:endIndex:progress:)]) {
        [self.delegate LZContentViewDidScroll:self startIndex:startIndex endIndex:endIndex progress:progress];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(LZContenViewDidEndDecelerating:startIndex:endIndex:)]) {
        [self.delegate LZContenViewDidEndDecelerating:self startIndex:startIndex endIndex:endIndex];
    }
}
...
{
    if (!decelerate) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(LZContenViewDidEndDragging:)]) {
            [self.delegate LZContenViewDidEndDragging:self];
        }
    }
}
