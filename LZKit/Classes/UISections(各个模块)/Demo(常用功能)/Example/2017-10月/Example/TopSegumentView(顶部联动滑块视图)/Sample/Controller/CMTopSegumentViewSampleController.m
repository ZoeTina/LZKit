...
- (void)initUI {
    ...
    CGFloat labelW = kScreenWidth - labelX*2;
    CGFloat labelH = [label.text lz_sizeWithFont:label.font maxWidth:labelW];
    ...
    CGRect tvFrame = CGRectMake(tableViewX, tableViewY, tableViewW, tableViewH);
    UITableView *tableView = [UITableView lz_initWithFrame:tvFrame
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dict) {
        ...
        if ([dict[@"classType"] isEqual:[MBProgressHUD class]]) {
            [LZUtilsToast showSuccessText:@"this is a test"];
            
        }else if([dict[@"classType"] isEqual:@""]){
            [LZUtilsToast showFailureText:@"功能暂未完善"];
