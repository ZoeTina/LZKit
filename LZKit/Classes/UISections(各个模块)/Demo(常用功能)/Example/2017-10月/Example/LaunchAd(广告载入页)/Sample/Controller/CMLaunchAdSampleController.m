...
//    [tableView registerClass:[LZHeaderView class] forHeaderFooterViewReuseIdentifier:@"LZHeaderView"];
    
    tableView.tableHeaderView = [LZHeaderView headView];
...
//    LZHeaderView *headView = [[LZHeaderView alloc] initWithReuseIdentifier:@"LZHeaderView"];
    LZHeaderView *headerView = [LZHeaderView headView];
    headerView.hight = 170;
    headerView.titleStr = @"官方示例为保证程序加载只显示一个载入广告，使用单例。但此处为了演示各个载入广告效果，取消了单例 \n实际使用注意:\n1️⃣请打开XHLaunchAd.m文件中shareLaunchAd方法的单例\n2️⃣或下载源码查看官方示例，源码地址：https://github.com/CoderZhuXH/XHLaunchAd";
