...
#define kAnimationDuration 1.5   // 绘制折线图时间
...
/** 手势接收View */
@property (strong, nonatomic) UIView * panGestureRecognizerView;

/** 数值标识竖线 */
@property (strong, nonatomic) UIView * touchFlowLineView;

/** 数值显示Label */
@property (strong, nonatomic) UILabel *touchNumLable;

/**  */
@property (assign, nonatomic) CGPoint  touchedDot;
...
- (void)handlePan:(UIGestureRecognizer *)recognizer {
    if (!_touchFlowLineView) {
        _touchFlowLineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1, self.bounds.size.height - self.contentInsets.bottom - self.contentInsets.top)];
        _touchFlowLineView.backgroundColor = [self.touchFlowLineColor colorWithAlphaComponent:0.5];
        [self addSubview:_touchFlowLineView];
    }

    
    if (!_touchNumLable) {
        _touchNumLable = [[UILabel alloc] init];
        _touchNumLable.font = [UIFont systemFontOfSize:11];
        _touchNumLable.textColor = [UIColor lz_colorWithHexString:@"666666"];
        _touchNumLable.backgroundColor = [UIColor clearColor];
        _touchNumLable.textAlignment  = NSTextAlignmentCenter;
   
        [self addSubview:_touchNumLable];
    }
    
    _touchNumLable.alpha  = 1.f;
    CGPoint translation = [recognizer locationInView:self];
    
    int touchDotIndex = [self closestDotFromVerticalLine:translation];
    NSValue *value = _firstDataArray[touchDotIndex];
    _touchedDot = value.CGPointValue;
    
    NSArray *info = _valueArr.firstObject;
    NSNumber *num = info[touchDotIndex];
    _touchNumLable.text = [NSString stringWithFormat:@"%@%.2f%@",_touchNumPrefixStr,num.floatValue,_touchNumSuffixStr];
    

//    NSValue *firstValue = _firstDataArray.firstObject;
//    NSValue *lastValue = _firstDataArray.lastObject;

    CGFloat X = 0;
    CGFloat Y = self.contentInsets.top + 20;
    CGFloat W = 50;
    CGFloat H = 20;
    _touchNumLable.frame = CGRectMake(X, Y, W, H);
    CGSize size = [_touchNumLable.text lz_sizeWithFont:[UIFont systemFontOfSize:11] maxSize:CGSizeMake(MAXFLOAT, H)];

    _touchFlowLineView.lz_x = _touchedDot.x;
    _touchNumLable.lz_x = _touchedDot.x - size.width/2;
    _touchNumLable.lz_y = _touchedDot.y - H + 5;
    [_touchNumLable sizeToFit];
    
//    // 设置提示竖线最大/最小边界值
//    if (translation.x <= lastValue.CGPointValue.x && translation.x >= firstValue.CGPointValue.x) {
//        _touchFlowLineView.x = translation.x;
//        X = translation.x;
//    }else if (translation.x > lastValue.CGPointValue.x){
//        _touchFlowLineView.x = lastValue.CGPointValue.x;
//        X = lastValue.CGPointValue.x;
//    }else{
//        _touchFlowLineView.x = firstValue.CGPointValue.x;
//        X = firstValue.CGPointValue.x;
//    }
...
- (int)closestDotFromVerticalLine:(CGPoint)touchPoint {
    
    int currentTouchTag  = INT_MAX;
    int dotIndex = 0;
    
    NSMutableArray *array = _drawDataArr.firstObject;
    
    for (int i=0;i<array.count;i++) {
        NSValue *value = array[i];
        CGPoint pointDot = value.CGPointValue;
        
        if (pow(((pointDot.x) - touchPoint.x), 2) < currentTouchTag) {
            
            currentTouchTag = pow(((pointDot.x) - touchPoint.x), 2);
            dotIndex = i;
...
- (void)configPanGestureRecognizerView {
    
    _panGestureRecognizerView = [[UIView alloc] initWithFrame:self.bounds];
    //    _panGestureRecognizerView.bottom = self.height - bottomHeight +25;
    _panGestureRecognizerView.backgroundColor = [UIColor clearColor];
    [self addSubview:_panGestureRecognizerView];
    
    // 单击手势
    UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handlePan:)];
    [_panGestureRecognizerView addGestureRecognizer:tapGestureRecognizer];
...
//    [_panGestureRecognizerView addGestureRecognizer:longG];
    
    
    // 第一个折线图数据
    _firstDataArray = _drawDataArr.firstObject;
...
-(instancetype)initWithFrame:(CGRect)frame andLineChartType:(JHLineChartType)lineChartType{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _touchFlowLineColor = [UIColor grayColor];
        ...
        case JHLineChartQuadrantTypeFirstQuardrant:{
            ...
            if (_yLineDataArr.count>0) {
                ...
                for (NSInteger i = 0; i<_yLineDataArr.count; i++) {
                    ...
                    double num = [_yLineDataArr[i] doubleValue];
                    NSString *numStr = [NSString string];
                    if (num >= 10000000) {
                        numStr = [NSString stringWithFormat:@"%dkw",(int)num/10000000];
                        [self drawText:numStr andContext:context atPoint:P_M(p.x-len+23, p.y-hei / 2) WithColor:_xAndYNumberColor andFontSize:self.yDescTextFontSize];
                    }else if (num >= 10000) {
                        numStr = [NSString stringWithFormat:@"%dw",(int)num/10000];
                        [self drawText:numStr andContext:context atPoint:P_M(p.x-len+11, p.y-hei / 2) WithColor:_xAndYNumberColor andFontSize:self.yDescTextFontSize];
                    }else if (num >=1000) {
//                        numDeal = num/10000;
                        numStr = [NSString stringWithFormat:@"%dk",(int)num/1000];
                        [self drawText:numStr andContext:context atPoint:P_M(p.x-len+5, p.y-hei / 2) WithColor:_xAndYNumberColor andFontSize:self.yDescTextFontSize];
                    }else{
//                        numDeal = num;
                        numStr = [NSString stringWithFormat:@"%@",_yLineDataArr[i] ];
                        [self drawText:numStr andContext:context atPoint:P_M(p.x-len-3, p.y-hei / 2) WithColor:_xAndYNumberColor andFontSize:self.yDescTextFontSize];
                    }
...
-(void)showAnimation{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kAnimationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self configPanGestureRecognizerView];
    });
...
//                   [secondPath moveToPoint:p];
    ...
    ani.duration = kAnimationDuration;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(ani.duration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (weakself.gradientColor.count  ==  weakself.drawDataArr.count) {
            gradLayer.colors = @[(__bridge id)[weakself.gradientColor[colorIndex] CGColor],(__bridge id)[[weakself.gradientColor[colorIndex] colorWithAlphaComponent:0.0] CGColor]];
            ...
            gradLayer.mask = shaperLay;
            [weakself.layer addSublayer:gradLayer];
            
        }else if(weakself.contentFillColorArr.count == weakself.drawDataArr.count){
...
//        [weakself.layer addSublayer:gradLayer];
////        [weakself.layer addSublayer:shaperLay];
        [self->_layerArr addObject:shaperLay];
    });
