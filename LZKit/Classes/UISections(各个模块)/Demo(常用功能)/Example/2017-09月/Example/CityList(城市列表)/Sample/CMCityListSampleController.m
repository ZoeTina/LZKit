...
- (void)initUI{
    ...
    [cityBtn lz_setTitleColor:kColorWithRGB(211, 0, 0)];
    [cityBtn lz_addTarget:self action:@selector(cityBtnClick:)];
...
{
    [[LZAlertView sharedInstance] showAlertController:self title:@"提示" message:@"选择城市列表类型" cancelTitle:@"取消" actionBlock:^(NSInteger buttonTag) {
        switch (buttonTag) {
            case 0: // 默认取消按钮
                break;
